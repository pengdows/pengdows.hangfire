...
[Collection("SqliteStress")]
public class SqliteLockStressFacts
{
    private readonly SqliteStressFixture _f;
    private readonly ITestOutputHelper _out;

    public SqliteLockStressFacts(SqliteStressFixture fixture, ITestOutputHelper output)
    {
        _f = fixture;
        _out = output;
    }
    ...
    [Fact(Timeout = 30_000)]
    public async Task MutualExclusion_20ConcurrentWorkers_ZeroOverlap_SqliteSingleWriter()
    {
        // SQLite is a single-process embedded database — realistic deployments have one
        // worker process with a handful of concurrent Hangfire threads, not hundreds.
        const int workerCount = 20;
        var resource  = "sqlite-stress-" + Guid.NewGuid().ToString("N");
        ...
        _out.WriteLine($"SQLite SingleWriter Stress (single-process): workers={workerCount}  acquired={acquired}  timeouts={timeouts}  violations={tracker.Violations}  maxConcurrent={tracker.GlobalMaxConcurrentOwners()}");

...
BEGIN
	...
	IF NOT EXISTS (SELECT [object_id] FROM [sys].[tables]
		WHERE [name] = 'hf_lock' AND [schema_id] = @SCHEMA_ID)
	BEGIN
		CREATE TABLE [HangFire].[hf_lock] (
			[resource]   NVARCHAR(100) NOT NULL,
			[owner_id]   NVARCHAR(40)  NOT NULL,
			[expires_at] DATETIME      NOT NULL,
			[version]    INT           NOT NULL DEFAULT 1,
			CONSTRAINT [PK_HangFire_hf_lock] PRIMARY KEY CLUSTERED ([resource] ASC)
		);
		PRINT 'Created table [HangFire].[hf_lock]';
	END
